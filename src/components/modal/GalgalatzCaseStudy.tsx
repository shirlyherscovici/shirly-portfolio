 ...
 *  artwork) by the designer — see FRAMES above. Restored per explicit
 *  direction (a later pass had cropped this down to a bezel-less flat
 *  tile; the phone frame is back, screens shown inside it as originally
 *  built). This just crossfades between the 5 flat images and keeps the
 *  same "tuning channels" scanline beat. No separate frame layer, no
  ...
  return (
    <div className="relative">
      ...
      <div className="px-5 sm:px-8 pb-6">
        ...
        <div className="relative w-full max-w-[520px] mx-auto lg:mx-0 lg:w-auto lg:max-w-none lg:h-full" style={{ aspectRatio: '765 / 680' }}>
          ...
          <div className="absolute" style={{ left: '60%', right: '0%', top: '0%', bottom: '0%' }}>
            ...
            <div className="relative h-full" style={{ perspective: 1400 }}>
              {/* Phone mockup restored to its original tilted, photographed
                  look (a later pass had flattened this to a plain
                  perspective-free tile) — the 5 screen mockups are already
                  fully composited onto the phone frame at its own native
                  canvas size (941×1672), so there's no separate frame +
                  content overlay to keep aligned: it's one flat image, and
                  the 3D tilt below is applied to that whole image at once. */}
              <motion.div
        ...
        {/* Added heading text above the strip is gone per explicit
            direction — just the film strip itself, restored to its
            original prominence (the added copy previously pushed a
            little visual weight onto text instead of the assets). */}
